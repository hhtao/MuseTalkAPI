...
def split_text_to_sentences(text: str, cut_method: str = "cut2") -> List[str]:
    ...
    try:
        ...
        # 获取并应用切分方法
        from .text_cut import get_method
        ...
        for sentence in result:
            ...
            if normalized and normalized not in normalized_content:
                normalized_content.add(normalized)
                unique_results.append(sentence)
        
        # 合并短句子，但保持冒号结构完整
        processed = []
        current_sentence = ""
        
        for i, sentence in enumerate(unique_results):
            # 检查是否包含冒号（结构化文本标志）
            has_colon = "：" in sentence or ":" in sentence
            is_last = i == len(unique_results) - 1
            ...
            # 只有在非冒号且句子很短时才考虑合并
            if not is_last and len(sentence.strip()) <= 5:
                current_sentence += sentence
            else:
                # 添加已处理的句子
                processed.append(current_sentence)
        ...
        # 验证内容完整性
        # 1. 检查结果中是否包含原始文本的最后部分
        # 2. 对于结构化的项目点列表，确保每个项目都被处理
        
        # 检查最后一个句子的完整性
        original_lines = original_text.split("\n")
        if original_lines and processed:
            # 获取原始文本最后一行的最后部分
            last_line = original_lines[-1].strip()
            last_part = last_line.split("，")[-1].split(",")[-1].strip()
            
            # 如果最后部分不在处理结果中，添加它
            last_found = False
            for sentence in processed:
                if last_part in sentence:
                    last_found = True
                    break
            
            if not last_found and last_part:
                logger.info("检测到可能缺失的结尾部分，添加: {}".format(last_part))
                processed.append(last_part)
        
        # 增加日志，方便调试
        logger.info("原始句子数: {}, 处理后句子数: {}".format(
            len(result), len(processed)))
